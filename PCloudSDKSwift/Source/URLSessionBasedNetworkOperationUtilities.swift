...
public struct URLSessionBasedNetworkOperationUtilities {
	...
	public static func createCallOperationBuilder(with scheme: URLScheme,
												  session: URLSession,
												  delegate: URLSessionEventHub) -> (Call.Request) -> CallOperation {
		return { request in
			let task = URLSessionTaskBuilder.createDataTask(with: request, session: session, scheme: scheme)
	...
	public static func createUploadOperationBuilder(with scheme: URLScheme,
													session: URLSession,
													delegate: URLSessionEventHub) -> (Upload.Request) -> UploadOperation {
		return { request in
			let task = URLSessionTaskBuilder.createUploadTask(with: request, session: session, scheme: scheme)
	...
	public static func createDownloadOperationBuilder(with session: URLSession,
													  delegate: URLSessionEventHub) -> (Download.Request) -> DownloadOperation {
		return { request in
			let task = URLSessionTaskBuilder.createDownloadTask(with: request, session: session)
